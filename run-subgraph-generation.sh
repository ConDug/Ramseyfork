...
start=$(date +%s.%N)
command="./maplesat-ks/simp/maplesat_static -order=$n -max-exhaustive-var=$((i*(i-1)/2)) -skip-last=$((n-i)) -exhaustive=$dir/$i-solo.exhaust -no-pre $f $f.drat -perm-out=$f.perm -keep-blocking=2 -noncanonical-out=$dir/$i-solo.noncanonical -minclause $p -no-pseudo-test  | tee $dir/subgraph-gen-solo-$i.log"
echo "$command"
eval "$command"
end=$(date +%s.%N)
